...
from Dataset2_processing import get_query
import normalize_doc
from Dataset2_processing import processing_Query, reading_RLE

nlp = spacy.load(
    r'C:\ProgramData\Anaconda3\Lib\site-packages\en_core_web_sm\en_core_web_sm-3.3.0')
nlp2 = spacy.load(
    r'C:\ProgramData\Anaconda3\Lib\site-packages\en_core_web_lg\en_core_web_lg-3.3.0')
...
def get_vector():
    ...
    for q in range(1, 10):
        ...
        for x in range(1, 1460):
            ...
            # contentAFile = f.read()
            # f.close()
            contentAFile = get_current_doc(x)
