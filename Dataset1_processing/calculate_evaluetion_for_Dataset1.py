...
from Dataset1_processing import get_dictionary_for_Dataset1, reading_RLE_Dataset1, get_query1
from Dataset2_processing import get_query, processing_Dataset2
...
def calculate():
    diction = get_dictionary_for_Dataset1.build_vec_mod()
    f3 = open("terms1.txt", "r")
    saved_terms = f3.read()
    f3.close()
    saved_terms = list(ast.literal_eval(saved_terms))

    for Q in range(1, 6):
        q = get_query1.get_current_query(Q)
